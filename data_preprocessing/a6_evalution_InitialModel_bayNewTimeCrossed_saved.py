    ...
    try:
        ...
        if nb_split_tscv < 2:
            ...
            for train_index, val_index in cv.split(X_train):
                ...
                unique_classes, class_counts = cp.unique(y_train_gpu, return_counts=True)
                class_weights = cp.sum(class_counts) / (len(unique_classes) * class_counts)
                sample_weights_gpu = class_weights[(y_train_gpu).astype(cp.int32)]

                # 4. Création des matrices DMatrix pour XGBoost
            ...
            objectives = calculate_normalized_objectives(
                ...
                scale_objectives=False, use_imbalance_penalty=True

            )


            print("\nObjectifs normalisés calculés :")
            ...
            #if max(scores_ens_val_list) > 500:
             #   print(scores_ens_val_list)
              #  print(f"Score {max(scores_ens_val_list):.1f} trop élevé (>500). Arrêt de l'entraînement.")
               # #exit(67)
...
if __name__ == "__main__":
    ...
    config = {
        ...
        'preShapImportance': 1,
        'cv_method': cv_config.TIME_SERIE_SPLIT, #cv_config.K_FOLD, #,
        'weightPareto_pnl_val': 0.85,
        'weightPareto_pnl_diff': 0.15,
    ...
    'num_boost_round': {'min': 100, 'max': 800},  # Réduit car early stopping à 44

    'max_depth': {'min': 3, 'max': 8},            # Resserré autour de 3 obtenu

    'learning_rate': {'min': 0.005, 'max': 0.1,    # Resserré autour de 0.072 obtenu
                     'log': True},

    'min_child_weight': {'min': 1, 'max': 7},     # Déplacé vers le haut car 5 obtenu

    'subsample': {'min': 0.4, 'max': 0.7},        # Déplacé vers le bas car 0.60 obtenu

    'colsample_bytree': {'min': 0.70, 'max': 0.95}, # Resserré autour de 0.83 obtenu

    'colsample_bylevel': {'min': 0.5, 'max': 0.8}, # Resserré autour de 0.68 obtenu

    'colsample_bynode': {'min': 0.5, 'max': 0.95}, # Resserré autour de 0.83 obtenu

    'gamma': {'min': 2, 'max': 7},               # Déplacé vers le haut car 9.54 obtenu

    'reg_alpha': {'min': 1, 'max': 11,             # Resserré autour de 6.14 obtenu
                 'log': True},

    'reg_lambda': {'min': 0.1, 'max': 3,          # Resserré autour de 0.53 obtenu
                  'log': True}
}
    ...
    df_filtered = sessions_selection(df, selected_sessions, CUSTOM_SECTIONS)
    ...
    selected_columnsByFiltering = [
        # Price action
        'diffPriceClosePoc_0_2',  # Différence entre close et POC -2
        'diffPriceClosePoc_0_3',
        'diffHighPrice_0_1',  # Différence entre high actuel et précédent
        'diffHighPrice_0_2',
        'diffHighPrice_0_3',
        'diffPriceCloseVWAP',  # Différence entre close et VWAP

        # Temporel et technique
        'deltaTimestampOpeningSection5min',  # Position temporelle en sections de 5min
        'perctBB',  # Position relative aux bandes de Bollinger
        'diffVolCandle_0_1Ratio',  # Ratio de différence de volume
        'diffVolDelta_0_1Ratio',  # Ratio de différence de delta
        'asymetrie_volume',  # Asymétrie du volume

        # Imbalances
        'bear_imbalance_high_1',  # Imbalances baissières high
        'bear_imbalance_high_2',
        'bear_imbalance_high_3',
        'bull_imbalance_high_0',  # Imbalances haussières high
        'bull_imbalance_high_1',
        'bull_imbalance_high_2',

        # Finished auction
        'finished_auction_high',  # Auction terminée en haut
        'finished_auction_low',  # Auction terminée en bas

        # Absorption et scores
        'bearish_repeatAskBid_ratio',  # Ratio repeat ask/bid baissier
        'bearish_ask_score',  # Score ask baissier
        'pocDeltaPocVolRatio',
        'ratioDeltaAbv',
        'candleSizeTicks',
