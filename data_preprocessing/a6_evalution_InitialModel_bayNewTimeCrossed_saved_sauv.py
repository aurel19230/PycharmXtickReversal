...
def objective_optuna(df_init=None, trial=None, study=None, X_train=None, X_test=None, y_train_label=None,
                     X_train_full=None,
                     device=None, xgb_param_optuna_range=None, config=None, nb_split_tscv=None,
    ...
    try:
        ...
        params, num_boost_round = setup_xgb_params(trial, xgb_param_optuna_range, random_state_seed_, device)
        ...
        cv_results = run_cross_validation_optuna(
...
def train_and_evaluate_XGBOOST_model(
    ...
    def callback_wrapper(study, trial):
        return callback_optuna(study, trial, optuna, study_xgb, rfe_param, config, xgb_param_optuna_range,
                               results_directory)
...
if __name__ == "__main__":
    ...
    config = {
        ...
        'constraint_min_trades_threshold_by_Fold': 10,
        'constraint_ecart_train_val': 0.16,
        'constraint_winrates_by_fold':0.51,
    ...
    xgb_param_optuna_range = {
        'num_boost_round': {'min': 400, 'max': 1000},  # Étendre vers le haut

        'max_depth': {'min': 4, 'max': 9},  # Étendre vers le haut

        'learning_rate': {'min': 0.001, 'max': 0.009,  # Resserrer autour de 0.025
                          'log': True},

        'min_child_weight': {'min': 1, 'max': 3},  # Resserrer vers le bas

        'subsample': {'min': 0.55, 'max': 0.75},  # Resserrer autour de 0.57

        'colsample_bytree': {'min': 0.65, 'max': 0.80},  # Resserrer vers le haut

        'colsample_bylevel': {'min': 0.4, 'max': 0.6},  # Étendre vers le bas

        'colsample_bynode': {'min': 0.65, 'max': 0.85},  # Resserrer vers le haut

        'gamma': {'min': 9, 'max': 17},  # Resserrer autour de 4.18

        'reg_alpha': {'min': 1, 'max': 2,  # Resserrer vers le bas
                      'log': True},

        'reg_lambda': {'min': 0.2, 'max': 1.2,  # Resserrer vers le bas
                       'log': True}
    }
