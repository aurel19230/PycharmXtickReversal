...
def objective_optuna(trial, study, X_train, y_train_label, X_train_full,
    ...
    if nb_split_tscv < 2:
        ...
        X_train_split = y_train_split = X_val_split = y_val_split = None
        dtrain = dval = None
        val_pred_proba = train_pred_proba = val_pred = train_pred = None

        # 1. Utiliser directement les données numpy
        X_train_full = np.array(X_train, dtype=np.float32)
        y_train_full = np.array(y_train_label, dtype=np.int32)

        # Boucle sur les splits
        for train_index, val_index in cv.split(X_train):
            # 2. Sélection des données
            X_train_split = X_train_full[train_index]
            y_train_split = y_train_full[train_index]
            X_val_split = X_train_full[val_index]
            y_val_split = y_train_full[val_index]

            # 3. Calcul des poids d'échantillons
            unique_classes, class_counts = np.unique(y_train_split, return_counts=True)
            class_weights = np.sum(class_counts) / (len(unique_classes) * class_counts)
            sample_weights = class_weights[(y_train_split).astype(np.int32)]

            # 4. Création des matrices DMatrix pour XGBoost
            dtrain = xgb.DMatrix(X_train_split, label=y_train_split, weight=sample_weights)
            ...
            if optima_score == optuna_options.USE_OPTIMA_CUSTOM_METRIC_PROFITBASED:
                custom_metric = lambda predtTrain, dtrain: custom_metric_ProfitBased_cpu(predtTrain, dtrain, metric_dict)
                obj_function = create_weighted_logistic_obj_cpu(w_p, w_n)
            ...
            try:
                ...
                # Obtenir y_val depuis dval
                y_val = dval.get_label()
                tn_val, fp_val, fn_val, tp_val = confusion_matrix(y_val, val_pred).ravel()
...
if __name__ == "__main__":
    ...
    #file_path = '/content/drive/MyDrive/Colab_Notebooks/Step5_4_0_4TP_1SL_080919_091024_extractOnly220LastFullSession_OnlyShort_feat_winsorized.csv'
    ...
    config = {
        ...
        'results_directory' : \
        (
            #r"/content/drive/MyDrive/Colab_Notebooks/xtickReversal/results_optim/"
    ...
    weight_param = {
        ...
        'std_penalty_factor': {'min': 0.95, 'max': 1.05}  # si 0, pas d'utilisation de weight_split
    ...
    excluded_columns = [
        ...
        'VolBlwState',#bas de classement shap
        ...
        'VolAbvState',#bas de classement shap
        ...
        'market_regimeADX_state',#bas de classement shap
