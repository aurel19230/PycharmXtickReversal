...
DROPOUT_RATE = 0.4
hidden_size = 256  # Increased from 256 to 512 (LSTM)
BIDIRECTIONAL = True
...
class CNNLSTMModel(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, num_layers=2, bidirectional=True):
        ...
        self.relu2 = nn.ReLU()
        self.lstm = nn.LSTM(128, hidden_size, num_layers=num_layers, batch_first=True, bidirectional=bidirectional)
    ...
    def forward(self, x):
        x = x.permute(0, 2, 1)  # Reshape for 1D convolution
        x = self.conv1(x)
        x = self.relu1(x)
        x = self.conv2(x)
        x = self.relu2(x)
...
# Instantiate model
model = CNNLSTMModel(X_sequences.shape[2], hidden_size, 3, num_layers=2, bidirectional=BIDIRECTIONAL)
model.to(DEVICE)
