...
class ScalerMode(Enum):
    ...
    TRANSFORM = 1      # Pour le test : transform uniquement
...
def plot_fp_tp_rates(X_test, y_test_label, y_test_predProba, feature_deltaTime_name,
                     optimal_threshold, dataset_name, user_input=None, results_directory=None):
    """
...
def train_finalModel_analyse(xgb=None,
    ...
    xgb_metric_method=config.get('xgb_metric_method', None)
    print(xgb_metric_method)
    if (xgb_metric_method==None):
        exit(13)

    # Configurer custom_metric et obj_function si nécessaire
    if xgb_metric_method == xgb_metric.XGB_METRIC_CUSTOM_METRIC_PROFITBASED:
        metric_dict = {
    ...
    if 'deltaTimestampOpeningSession1min' in X_test.columns:
        plot_fp_tp_rates(X_test=X_train, y_test_label=y_train_label, y_test_predProba=y_train_predProba,
                         ...
                         dataset_name="train", user_input=user_input,  results_directory=results_directory)
        plot_fp_tp_rates(X_test=X_test, y_test_label=y_test_label, y_test_predProba=y_test_predProba,
                         feature_deltaTime_name='deltaTimestampOpeningSession1min',optimal_threshold=optimal_threshold,
                         dataset_name="test", user_input=user_input, results_directory=results_directory)
...
def setup_model_params(trial, modele_param_optuna_range, random_state_seed_, device):
    """Configure les paramètres XGBoost pour l'entraînement"""
    params = {
    ...
    # num_boost_round est retourné séparément
...
def process_cv_fold(X_train=None, X_train_full=None, fold_num=None, train_pos=None, val_pos=None, params=None, num_boost_round=None,
                    data_gpu=None, metric_dict=None, trial=None, weight_param=None, optima_score=None, is_log_enabled = False):
    try:
        # Debug initial
        # print(f"\n=== Debug Fold {fold_num} ===")

        # Extraction et vérification des données
        X_train_gpu = data_gpu['X_train_gpu_full'][train_pos]
        y_train_gpu = data_gpu['y_train_gpu_full'][train_pos]
        X_val_gpu = data_gpu['X_train_gpu_full'][val_pos]
        y_val_gpu = data_gpu['y_train_gpu_full'][val_pos]
...
"""
def run_cross_validation_optuna(X_train, y_train_label, trial, params, num_boost_round, metric_dict, cv, nb_split_tscv,
                                weight_param, optima_score, xgb_metric, device):
    try:
        #print("\n=== Début nouvelle validation croisée ===")
        ...
        if isinstance(y_train_label, pd.Series):
            y_train_label = y_train_label.reset_index(drop=True)

        # Initialisation des métriques
        metrics_dict = initialize_metrics_dict(nb_split_tscv)
        all_fold_stats = {}

        # Initialisation des arrays GPU
        winrates_by_fold = cp.zeros(nb_split_tscv, dtype=cp.float32)
        nb_trades_by_fold = cp.zeros(nb_split_tscv, dtype=cp.int32)
        scores_train_by_fold = cp.zeros(nb_split_tscv, dtype=cp.float32)
        tp_train_by_fold = cp.zeros(nb_split_tscv, dtype=cp.float32)
        fp_train_by_fold = cp.zeros(nb_split_tscv, dtype=cp.float32)
        tp_val_by_fold = cp.zeros(nb_split_tscv, dtype=cp.float32)
        fp_val_by_fold = cp.zeros(nb_split_tscv, dtype=cp.float32)
        scores_val_by_fold = cp.zeros(nb_split_tscv, dtype=cp.float32)
        ...
        # Boucle CV avec vérification des indices
        for fold_num, (train_pos, val_pos) in enumerate(cv.split(X_train)):
            print(f"\nFold {fold_num + 1}/{nb_split_tscv}")
            print(f"Train index range: {min(train_pos)}-{max(train_pos)}")
            print(f"Val index range: {min(val_pos)}-{max(val_pos)}")

            if len(train_pos) == 0 or len(val_pos) == 0:
                print("⚠️ Ensemble vide détecté")
                continue
            ...
            print("Distribution train:", np.unique(y_train_fold, return_counts=True))
            print("Distribution val:", np.unique(y_val_fold, return_counts=True))

            fold_results = process_cv_fold(
                fold_num=fold_num,
                train_pos=train_pos,
                val_pos=val_pos,
                params=params,
            ...
            xgboost_train_time_cum += fold_results['training_time']

            # Vérification des résultats du fold
            #print("\nVérification des métriques du fold:")
            #print(f"Score val: {fold_results['val_metrics']['score']}")
            #print(f"TP val: {fold_results['val_metrics']['tp']}, FP val: {fold_results['val_metrics']['fp']}")
            #print(f"Winrate: {fold_results['fold_stats']['val_winrate']}")
            ...
            # Stockage des stats et métriques
            all_fold_stats[fold_num] = fold_results['fold_stats']
            winrates_by_fold[fold_num] = fold_results['fold_stats']['val_winrate']
            nb_trades_by_fold[fold_num] = fold_results['fold_stats']['val_trades']
            tp_train_by_fold[fold_num] = fold_results['train_metrics']['tp']
            fp_train_by_fold[fold_num] = fold_results['train_metrics']['fp']
            tp_val_by_fold[fold_num] = fold_results['val_metrics']['tp']
            fp_val_by_fold[fold_num] = fold_results['val_metrics']['fp']
            scores_val_by_fold[fold_num] = fold_results['val_metrics']['score']
            scores_train_by_fold[fold_num] = fold_results['train_metrics']['score']

            # Vérification des mises à jour
            #print("\nVérification des arrays après mise à jour:")
            arrays_to_check = {
                'winrates_by_fold': winrates_by_fold,
                'nb_trades_by_fold': nb_trades_by_fold,
        ...
        # Calcul des métriques finales avec vérification
        ...
        mean_val_score = np.mean(cp.asnumpy(scores_val_by_fold))
        std_val_score = np.std(cp.asnumpy(scores_val_by_fold))
        #print(f"\nScore moyen: {mean_val_score:.4f} ± {std_val_score:.4f}")
        ...
        results = {
            ...
            'scores_val_by_fold': scores_val_by_fold,
            'training_time': xgboost_train_time_cum,
        ...
        print(f"Erreur détaillée dans run_cross_validation_optuna:")
...
def run_cross_validation_optuna(X_train,X_train_full, y_train_label, trial, params, num_boost_round, metric_dict, cv, nb_split_tscv,
                                weight_param, optima_score, xgb_metric, device,is_log_enabled):
    try:
        print_notification("\n=== Début nouvelle validation croisée ===")
        print(f"Nombre de features apres redressement:  {len(X_train.columns)}\n:: {list(X_train.columns)}")
        ...
        for fold_num, (train_pos, val_pos) in enumerate(cv.split(X_train)):
            ...
            assert all(train_pos[i] < train_pos[i + 1] for i in range(len(train_pos) - 1))
            assert all(val_pos[i] < val_pos[i + 1] for i in range(len(val_pos) - 1))

            # Vérifier que validation vient après entrainement
            ...
            fold_results = process_cv_fold(X_train=X_train,X_train_full=X_train_full,
                fold_num=fold_num,
                train_pos=train_pos,
                val_pos=val_pos,
                params=params,
...
def callback_optuna(study, trial,optuna,study_xgb,rfe_param,config,modele_param_optuna_range,results_directory):
    """
    ...
    method_names_xgb_metric=method_names_xgb_metric.get(config.get("xgb_metric_method",None))
    ...
    if  config['use_optuna_constraints_func']==True:
        if contraints_reached==True:
            print(f"    =>best trial have a constraint ecart_train_val of : {best_trial.user_attrs.get('ecart_train_val', None)} (ecart_train_val:{config['constraint_ecart_train_val']} | "
                  f"constraint_min_trades_threshold_by_Fold: {config['constraint_min_trades_threshold_by_Fold']}) ")
        else:
            print(
                f"{Fore.RED}    =>WARNING: No feasible trials yet - all current trials violate the constraint (ecart_train_val > {config['constraint_ecart_train_val']}){Style.RESET_ALL}")
