...
if indicator_type == "atr":
    ...
    ATR_LOW_THRESHOLD_L = 2
    ATR_LOW_THRESHOLD_H = 3

    ATR_HIGH_THRESHOLD_L = 2
    ATR_HIGH_THRESHOLD_H = 3
...
if indicator_type == "vwap":
    ...
    VWAP_LOW_L = -20# Limite inférieure pour le seuil bas
    VWAP_LOW_H =  20  # Limite supérieure pour le seuil bas

    VWAP_HIGH_L = -20  # Limite inférieure pour le seuil haut
    VWAP_HIGH_H =  20  # Limite supérieure pour le seuil haut
...
if indicator_type == "williams_r":
    OPTIMIZE_OVERSOLD = False  # Set to False to disable oversold zone optimization => => find the worst winrate
    OPTIMIZE_OVERBOUGHT = True  # Set to False to disable overbought zone optimization => optimize de winrate
    SPLIT_SCORE_VAL=0.2 #0.6 represente que le score val compte pour 60% et 40% pour train

    # Définir des constantes globales pour les contraintes
    MIN_BIN_SPREAD = 0.03  # Écart minimum entre bins
    MAX_BIN_0_WIN_RATE = 0.475  # Maximum pour bin0 (doit être < 0.5)
    MAX_BIN_1_WIN_RATE = 0.529  # Minimum pour bin1 (doit être > 0.5)
    MIN_BIN_SIZE_0 = 0.000011  # Taille minimale pour un bin individuel
...
if indicator_type == "mfi":
    ...
    MAX_BIN_0_WIN_RATE = 0.475  # Maximum pour bin0 (doit être < 0.5)
    MAX_BIN_1_WIN_RATE = 0.52  # Minimum pour bin1 (doit être > 0.5)
    MIN_BIN_SIZE_0 = 0.115  # Taille minimale pour un bin individuel
    MIN_BIN_SIZE_1 = 0.14 # Taille minimale pour un bin individuel
    COEFF_SPREAD = 0.6
    COEFF_BIN_SIZE = 0.4
...
if indicator_type == "mfi_divergence":
    ...
    MIN_MFI_DECREASE_L = 0.00008
    MIN_MFI_DECREASE_H = 0.001
...
if indicator_type == "percent_bb_simu":
    ...
    # Définir des constantes globales pour les contraintes
    MIN_BIN_SPREAD = 0.0501  # Écart minimum entre bins
    MAX_BIN_0_WIN_RATE = 0.485  # Maximum pour bin0
    MAX_BIN_1_WIN_RATE = 0.505  # Minimum pour bin1
    MIN_BIN_SIZE_0 = 0.09  # Taille minimale pour bin0
    MIN_BIN_SIZE_1 = 0.08  # Taille minimale pour bin1
    COEFF_SPREAD = 1
    COEFF_BIN_SIZE = 0

    # Paramètres spécifiques à l'indicateur
    PERCTBB_PERIOD_L = 100
    PERCTBB_PERIOD_H = 120
...
def objective_perctBB_simu_modified(trial, df_train_, df_val_):
    """
    Fonction objective optimisée pour Optuna qui ajuste les paramètres de l'indicateur
    ...
    # --- 2.1 Calcul du %B sur TRAIN
    percent_b_values_train = calculate_percent_bb(
        df=df_train_, period=period, std_dev=std_dev, fill_value=0, return_array=True
    )

    # --- 2.2 Créer les indicateurs avec une logique adaptée au mode d'optimisation
    if OPTIMIZE_OVERBOUGHT:
        df_train_['is_bb_range'] = np.where(
            (percent_b_values_train >= bb_high_threshold),
            1, 0
        )

    if OPTIMIZE_OVERSOLD:
        df_train_['is_bb_extrem'] = np.where(
            (percent_b_values_train <= bb_low_threshold),
            1, 0
        )
    ...
    try:
        ...
        if OPTIMIZE_OVERSOLD:
            oversold_df_train = df_train_filtered[df_train_filtered['is_bb_extrem'] == 1]
            if len(oversold_df_train) == 0:
                return -np.inf  # Pas de trades, on pénalise
            bin_0_win_rate_train = oversold_df_train['class_binaire'].mean()
            bin_0_pct_train = len(oversold_df_train) / len(df_train_filtered)
        ...
        if OPTIMIZE_OVERBOUGHT:
            overbought_df_train = df_train_filtered[df_train_filtered['is_bb_range'] == 1]
            if len(overbought_df_train) == 0:
                return -np.inf
            bin_1_win_rate_train = overbought_df_train['class_binaire'].mean()
            bin_1_pct_train = len(overbought_df_train) / len(df_train_filtered)
    ...
    # --- 3.1 Calcul du %B sur VALIDATION
    percent_b_values_val = calculate_percent_bb(
        df=df_val_, period=period, std_dev=std_dev, fill_value=0, return_array=True
    )

    # --- 3.2 Créer les indicateurs pour VALIDATION
    if OPTIMIZE_OVERBOUGHT:
        df_val_['is_bb_range'] = np.where(
            (percent_b_values_val >= bb_high_threshold),
            1, 0
        )

    if OPTIMIZE_OVERSOLD:
        df_val_['is_bb_extrem'] = np.where(
            (percent_b_values_val <= bb_low_threshold),
            1, 0
        )
    ...
    try:
        if OPTIMIZE_OVERSOLD:
            oversold_df_val = df_val_filtered[df_val_filtered['is_bb_extrem'] == 1]
            if len(oversold_df_val) == 0:
                return -np.inf
            bin_0_win_rate_val = oversold_df_val['class_binaire'].mean()
            bin_0_pct_val = len(oversold_df_val) / len(df_val_filtered)
        ...
        if OPTIMIZE_OVERBOUGHT:
            overbought_df_val = df_val_filtered[df_val_filtered['is_bb_range'] == 1]
            if len(overbought_df_val) == 0:
                return -np.inf
            bin_1_win_rate_val = overbought_df_val['class_binaire'].mean()
            bin_1_pct_val = len(overbought_df_val) / len(df_val_filtered)
        ...
        print(f"📊 Couverture Validation (Bin 1) : {bin_1_pct_val:.2f}%")
        print(f"✅ Win Rate Validation (Bin 1) : {bin_1_win_rate_val:.4f}")
